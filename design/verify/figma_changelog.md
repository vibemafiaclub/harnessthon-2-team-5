...
`shots/` 에는 화면 24장 외에 조망·증거용 캡처가 있다. `index.md` 원장에는 포함되지 않는다.

| 파일 | 성격 | 갱신 |
|---|---|---|
| `overview_all24.png` (3970×2915) | Screens 페이지 전체 조망 — 24장을 한 화면에서 본다 | **최신** |
| `all_24.png` · `screens_all.png` | 같은 조망의 사본 | **최신**(overview 와 동일) |
| `components.png` (1590×1058) | Components 페이지 전체 | **최신** |
| `a7_case_bad.png` | A-7 합성 테스트 케이스 | **낡은 채로 둔다** |
| `tabbar_contacts.png` · `tabbar_meeting.png` | 아이콘 결함 지적 당시의 탭바 | **낡은 채로 둔다** |

뒤 3장은 **그 시점 상태의 증거**다. 갱신하면 무엇이 문제였는지 보여 주는 기능을 잃는다.
