...
| 01_home | C-6 | local | 처리 안 함 | F-2. 목록 마지막 행(y=694-704)이 불투명 CTA 바(top y=705)와 맞닿는다. 실제 겹침 0px 이라 잘림은 아니고 스크림 부재는 취향 영역이다. 다만 같은 화면의 오타 '서배'→'선배' 는 실제 결함이며 아직 안 고쳤다 |
...
| 06_groupcompose | C-1 | local | 처리 안 함 | F-9. 비활성 CTA 표현이 06·07 두 화면에서 다르다. 아직 수정하지 않았다. Button/Primary 의 Disabled variant 를 한쪽으로 통일하면 해소되며 근거도 있으나(§4 일관성 RULE), 이번 런의 남은 예산을 A-14 검증에 썼다 |
| 07_datepropose | C-8 | local | 처리 안 함 | F-4 의 같은 결함. 05_contactedit 과 동일 사유로 보류 |
| 07_datepropose | C-1 | local | 처리 안 함 | F-9 의 같은 결함. 06_groupcompose 와 동일 사유로 보류 |
...
- fail 13행(구 9건) 중 **수정 1건**(F-1 / 03_guestreply C-6), **처리 안 함 12행**
