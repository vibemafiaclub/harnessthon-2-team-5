...
/* Z-8 */
{ const lines = txt.split('\n').length, lmax = cap('agent_prd_analysis_lines_max', 120); const s5 = sec(5) || ''; const open = s5.split('\n').filter((l) => /^\s*(-|\d+\.|\|)/.test(l) && !/^\s*\|\s*-/.test(l) && !/^\s*\|\s*#/.test(l)).length; const omax = cap('agent_prd_open_questions_max', 5); const sc = new Set((txt.match(/\bS-\d+\b/g) || [])).size, smax = cap('agent_scenarios_max', 3);
