...
const out = {
  ...
  rules: [
    ...
    /* 네이밍은 두 규칙으로 나눈다(D-48 후속): applicable 집계가 맞아야 한다 — allow 는 컨테이너에만, deny 는 전 노드에. 한 규칙에 두 필터를 섞으면 applicable 이 전 노드로 잡혀 위반 수와 방향이 안 맞는다(test2 실측 1172 vs 533). */
    { id: 'layer-naming-semantic', title: '레이어 네이밍 semantic (컨테이너)', stage: ['wireframe', 'design'], severity: 'warning', applies_to: { node_types: ['FRAME', 'COMPONENT', 'COMPONENT_SET', 'INSTANCE'], descendants_only: true, exclude_instance_children: true },
      check: { type: 'name_pattern', allow: '^[A-Z][^/]*(/[^/]+)*$', allow_node_types: ['FRAME', 'COMPONENT', 'COMPONENT_SET', 'INSTANCE'] }, autofix: false, status: 'filled',
      source: '3-D 5 정규식(C안, test2 실측 533 컨테이너 전수 비교): 첫 세그먼트만 대문자 시작, 슬래시 뒤 세그먼트·한글·인스턴스 기본 이름(Row/Meeting - 대학 동기 모임)·Tab/home 허용. person-row·chip·btn-ok 같은 소문자 코드형 이름은 위반',
      fix_hint: '역할 기반 이름(Card/MeetingRow, Chip/Status, Tab/home)으로. 첫 글자만 대문자면 된다 — 세그먼트 뒤 내용은 자유(한글 가능).' },
    { id: 'layer-naming-auto', title: '자동 생성 이름 없음', stage: ['wireframe', 'design'], severity: 'warning', applies_to: {},
      ...
      fix_hint: 'Frame 12·Rectangle 3 같은 자동 이름은 역할 이름으로 바꾼다.' },
