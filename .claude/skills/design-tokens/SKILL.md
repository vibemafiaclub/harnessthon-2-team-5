...
- [ ] **K-1** `design/tokens.json` 유효 JSON, 빈 문자열 값 0개(`_note`·`$schema_note`·`_reference_note` 제외, `meta.reference_system` 이 null 이면 `meta.reference_palette` 도 제외), 6카테고리 `rationale` 전부 비어 있지 않음, `typography.family.fallback` 존재
