...
**미감 정합 3항**(`M-1`·`M-2`·`M-3`) — c_checks 의 바닥선 중 토큰 값만으로 판정되는 셋을 Figma 3단계까지 미루지 않고 여기서 거른다. `check-tokens.js` K-3 이 `token_sets.json` 에서 **직접 재계산**한다(wcag_tokens.md 의 `| M-n | PASS |` 행은 기록이지 증거가 아니다). 아래 명령은 사람이 항목별로 다시 볼 때의 원문:
